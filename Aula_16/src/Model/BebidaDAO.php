...
class BebidaDAO {
    ...
    // READ - Este é o método que substitui o getAll()
    ...
    // Método save para compatibilidade (usado no create)
    public function save(Bebida $bebida) {
        $this->criarBebida($bebida);
    }

    // Método deleteByName para compatibilidade (usado no delete)
    public function deleteByName($nome) {
        $this->excluirBebida($nome);
    }

    // Método getAllAsArray para compatibilidade (se necessário)
    private function getAllAsArray() {
        $bebidas = $this->lerBebidas();
        $result = [];
        foreach ($bebidas as $bebida) {
            $result[] = $bebida->toArray();
        }
        return $result;
    }
}